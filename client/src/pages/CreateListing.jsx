    ...
    const storeImage = async (file) => {
        return new Promise((resolve, reject) => {
            ...
            uploadTask.on(
                ...
                () => {
                    getDownloadURL(uploadTask.snapshot.ref).then((downloadUURL) => {
                        resolve(downloadUURL);
                    })
                }
            );
        });
            ...
            <form className='flex flex-col sm:flex-row gap-4'>
                <div className='flex flex-col gap-4 flex-1'>
                    ...
                    <div className='flex flex-wrap gap-6'>
                        ...
                        <div className="flex items-center gap-2">
                            <input 
                                type="number" id='discountedPrice' min={1} max={10} required
                                className='p-3 border border-gray-300 rounded-lg'
                            />
                            <div className="flex flex-col items-center">
                        ...
                        formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                            <div key={url} className="flex justify-between p-3 items-center">
                                ...
                                <button 
                                    ...
                                    className="p-3 text-red-700 uppercase hover:opacity-75">Delete</button>
                            </div>
                        ))
                    }
                    <button className='p-3 bg-gray-700 text-white 
                        uppercase rounded-lg hover:opacity-95 
                        disabled: opacity-80'> Create Listing
                    </button>
                </div>
